...
class PlanController extends BaseController
{
    ...
    public function index(Curl $curl)
    {
        if (Session::get('token') != null) {
            if ((session()->get('role_id') == 34) || \Utility::checkPermission("plan.index")) {
                try {
                    $result = $this->planService->index();
    ...
    public function filter(Request $request, Curl $curl)
    {
        ...
        try {
            $data = $request->all();
            $result = $this->planService->filter($data);
    ...
    {
        if ((session()->get('role_id') == 34) || \Utility::checkPermission("plan.create")) {
    ...
    public function store(Request $request)
    {
        ...
        try {
            $data = $request->all();
            $result = $this->planService->create($data);
            // dd($result);
            if ($result['status'] == true) {
    ...
    {
        if ((session()->get('role_id') == 34) || \Utility::checkPermission("plan.edit")) {
            try {
                $result = $this->planService->edit($id);
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            $result = $this->planService->update($data, $id);
            //dd($data);
            if ($result['status'] == true) {
                self::message('success', $result['message']);
                return redirect('plan');
    ...
    {
        if ((session()->get('role_id') == 34) || \Utility::checkPermission("plan.show")) {
            $getResponse = $curl->send('GET', 'plan', $id, '', 'display');
            return view('admin.pages.plan.details', [
                'prefixname' => 'Admin',
                'title' => 'User Create',
    ...
    {
        if ((session()->get('role_id') == 34) || \Utility::checkPermission("plan.destroy")) {
            try {
                $getResponse = $curl->send('DELETE', 'plan', $id, '', 'delete');
                if ($getResponse) {
