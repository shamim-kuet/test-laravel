...
class SuburbsService
{
	...
	public function __construct(Curl $curl)
    {
	 ...
	 public function index()
     {
        ...
        try {
			 $response = $this->curl->send('GET','suburbs','', '','display');
            // dd($response);
    ...
    public function create($inputs)
    {
        ...
        try {
			$response = $this->curl->send('POST','suburbs/store', '', $inputs,'insert');
			// dd($response);
            if ($response->success == true) {
    ...
    {
        try {
			 $response = $this->curl->send('GET','suburbs', $id,'','display');
            //  dd($response);
            if ($response->success == true) {
				$data = array('status' => true, 'message' => $response->message, 'data' => $response->data);
				return $data;
	...
	public function update($inputs, $id)
    {
        ...
        try {
			 $response = $this->curl->send('POST','suburbs/update', $id, $inputs,'update');
			 //dd($response);
            if ($response->success == true) {
				$data = array('status' => true, 'message' => $response->message);
				return $data;
