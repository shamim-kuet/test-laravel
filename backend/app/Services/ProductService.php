...
class ProductService
{
	...
	public function __construct(Curl $curl)
    {
     ...
     {
        try {
			 $response = $this->curl->send('GET','product','', '','display');
			//  dd($response);
	...
	public function filter($inputs)
    {
        // dd($inputs);
        try {
			 $response = $this->curl->send('POST','product-filter','',$inputs,'filter');
			//  dd($response);
            if ($response->success == true) {
				$data = array('status' => true, 'message' => $response->message, 'data' => $response->data);
				return $data;
    ...
    public function create($inputs)
    {
        ...
        try {
			//dd($inputs);
			$response = $this->curl->send('POST','product/store', '', $inputs,'insert');
            return $response;
			//dd($response);
            // if ($response->success == true) {
    ...
    {
        try {
			 $response = $this->curl->send('GET','product', $id,'','display');
            if ($response->success == true) {
				$data = array('status' => true, 'message' => $response->message, 'data' => $response->data);
				return $data;
            } else {
	...
	public function update($inputs, $id)
    {
        ...
        try {
			 $response = $this->curl->send('POST','product/update', $id, $inputs,'update');
			 //dd($response);
            if ($response->success == true) {
				$data = array('status' => true, 'message' => $response->message);
				return $data;
