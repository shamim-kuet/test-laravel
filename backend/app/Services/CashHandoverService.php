...
class CashHandoverService
{
	...
	public function __construct(Curl $curl)
    {
     ...
     {
        try {
			 $response = $this->curl->send('GET','cashhandover','', '','display');
			 //dd($response);
    ...
    public function create($inputs)
    {
        ...
        try {
			//dd($inputs);
			$response = $this->curl->send('POST','cashhandover/store', '', $inputs,'insert');
			//dd($response);
            if ($response->success == true) {
	...
	public function update($inputs)
    {
		...
		//dd($inputs);
        try {
			 $response = $this->curl->send('POST','cashhandover/update', '', json_encode($inputs),'update');
			 //dd($response);
            if ($response->success == true) {
				$data = array('status' => true, 'message' => $response->message);
				return $data;
