...
class CodchargeService
{
	...
	public function __construct(Curl $curl)
    {
	 ...
	 public function index()
     {
        ...
        try {
			 $response = $this->curl->send('GET','codcharge','', '','display');
    ...
    {
        try {
			 $response = $this->curl->send('POST','codcharge-filter','',$inputs,'filter');
			//  dd($response);
            if ($response->success == true) {
				$data = array('status' => true, 'message' => $response->message, 'data' => $response->data);
				return $data;
    ...
    public function create($inputs)
    {
        ...
        try {
			$response = $this->curl->send('POST','codcharge/store', '', $inputs,'insert');
			//dd($response);
            if ($response->success == true) {
    ...
    {
        try {
			 $response = $this->curl->send('GET','codcharge', $id,'','display');
            //  dd($response);
            if ($response->success == true) {
				$data = array('status' => true, 'message' => $response->message, 'data' => $response->data);
				return $data;
	...
	public function update($inputs, $id)
    {
        ...
        try {
			 $response = $this->curl->send('POST','codcharge/update', $id, $inputs,'update');
			 //dd($response);
            if ($response->success == true) {
				$data = array('status' => true, 'message' => $response->message);
				return $data;
