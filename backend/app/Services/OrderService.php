...
class OrderService
{
    ...
    public function __construct(Curl $curl)
    {
     ...
     {
         try {
             $response = $this->curl->send('GET', 'order', '', '', 'display');
             //  dd($response);
    ...
    public function filter($inputs)
    {
        // dd($inputs);
        try {
            $response = $this->curl->send('POST', 'order-filter', '', $inputs, 'filter');
            //  dd($response);
            if ($response->success == true) {
                $data = array('status' => true, 'message' => $response->message, 'data' => $response->data);
                return $data;
    ...
    public function create($inputs)
    {
        ...
        $inputs['partner_id'] = 1;
        try {
            $response = $this->curl->send('POST', 'order/store', '', json_encode($inputs), 'insert');
            // dd($response);
            if ($response->success == true) {
    ...
    {
        try {
            $response = $this->curl->send('GET', 'order', $id, '', 'display');
            if ($response->success == true) {
                $data = array('status' => true, 'message' => $response->message, 'data' => $response->data);
                return $data;
            } else {
    ...
    public function update($inputs, $id)
    {
        ...
        try {
            $response = $this->curl->send('POST', 'order/update', $id, json_encode($inputs), 'update');
            //  dd($response);
            if ($response->success == true) {
                $data = array('status' => true, 'message' => $response->message);
                return $data;
