...
class DocumentService
{
	...
	public function __construct(Curl $curl)
    {
     ...
     {
        try {
			 $response = $this->curl->send('GET','document','', '','display');
			// dd($response);
    ...
    {
        try {
			 $response = $this->curl->send('POST','document-filter','',$inputs,'filter');
            //  dd($response);
            if ($response->success == true) {
				$data = array('status' => true, 'message' => $response->message, 'data' => $response->data);
				return $data;
    ...
    public function create($inputs)
    {
        ...
        try {
			//dd($inputs);
			$response = $this->curl->send('POST','document/store', '', $inputs,'insert');
			//dd($response);
            if ($response->success == true) {
    ...
    {
        try {
			 $response = $this->curl->send('GET','document', $id,'','display');
	...
	public function update($inputs, $id)
    {
		...
		$inputs['partner_id'] = 1;
        try {
			 $response = $this->curl->send('POST','document/update', $id, $inputs,'update');
			 //dd($response);
            if ($response->success == true) {
				$data = array('status' => true, 'message' => $response->message);
				return $data;
