...
class EmployeeidService
{
	...
	public function __construct(Curl $curl)
    {
	 ...
	 public function index()
     {
        ...
        try {
			 $response = $this->curl->send('GET','employeeid','', '','display');
                //dd($response);
    ...
    {
        try {
			 $response = $this->curl->send('POST','employeeid-filter','',$inputs,'filter');
			//  dd($response);
            if ($response->success == true) {
				$data = array('status' => true, 'message' => $response->message, 'data' => $response->data);
				return $data;
    ...
    public function create($inputs)
    {
        ...
        try {
			$response = $this->curl->send('POST','employeeid/store', '', $inputs,'insert');
			//dd($response);
            if ($response->success == true) {
    ...
    {
        try {
			 $response = $this->curl->send('GET','employeeid', $id,'','display');
            if ($response->success == true) {
				$data = array('status' => true, 'message' => $response->message, 'data' => $response->data);
				return $data;
            } else {
	...
	public function update($inputs, $id)
    {
        ...
        try {
			 $response = $this->curl->send('POST','employeeid/update', $id, $inputs,'update');
			 //dd($response);
            if ($response->success == true) {
				$data = array('status' => true, 'message' => $response->message);
				return $data;
