...
class PlanService
{
	...
	public function __construct(Curl $curl)
    {
     ...
     {
        try {
			 $response = $this->curl->send('GET','plan','', '','display');
    ...
    public function create($inputs)
    {
        ...
        try {
			$response = $this->curl->send('POST','plan/store', '', $inputs,'insert');
			// dd($response);
            if ($response->success == true) {
    ...
    {
        try {
			 $response = $this->curl->send('GET','plan', $id,'','display');
            if ($response->success == true) {
				$data = array('status' => true, 'message' => $response->message, 'data' => $response->data);
				return $data;
            } else {
	...
	public function update($inputs, $id)
    {
        ...
        try {
			 $response = $this->curl->send('POST','plan/update', $id, $inputs,'update');
			 //dd($response);
            if ($response->success == true) {
				$data = array('status' => true, 'message' => $response->message);
				return $data;
